...
contract ERC20Radiate is ERC20, ERC20Burnable, CurvedAccessControl {
    ...
    * @dev The operational rate, which determines how much of the reserve is used
    ...
    function shareRate(address _caller) public view returns (uint256) {
        if (tokenSupply_ > 0) {
            return (balanceLedger_[_caller] * magnitude) / tokenSupply_;
        } else {
            return 0;
        }
    }
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal override {
        ...
        // Perform swap
        balanceLedger_[_from] = balanceLedger_[_from].sub(_taxedRadiateShares);

        balanceLedger_[_to] = balanceLedger_[_to].add(_taxedRadiateShares);
    ...
    function wrapTokens(address _receiver, uint256 _tokens) internal returns (uint256) {
        ...
        // TODO - Update allowance checks to support V3 router
        /*
        ...
        */
        ...
        tokenSupply_ = tokenSupply_.add(_taxedRadiateTokens);
